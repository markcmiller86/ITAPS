...
bool tag_info_test(TSTTG::Geometry &geom) 
{
  ...
  try {
    geom_tag.createTag(tag_name, 4, TSTTB::TagValueType_OPAQUE, this_tag);
  }
  catch (TSTTB::Error) {
  ...
  int tag_size;
  try {
    tag_size = geom_tag.getTagSize(this_tag);
  }
  catch (TSTTB::Error) {
  ...
  for (dim = 0; dim <= 3; dim++) {
    ...
    try {
      geom_atag.setArrData(gentity_handles, num_gentity_handles,
                           this_tag, tag_vals, tag_vals_size, sizeof(int));
    }
    catch (TSTTB::Error) {
  ...
  for (dim = 0; dim <= 3; dim++) {
    ...
    try {
      geom_atag.getArrData(gentity_handles, num_gentity_handles,
                           this_tag, tag_vals, tag_vals_size, siz);
    }
    catch (TSTTB::Error) {
...
bool booleans_test(TSTTG::Modify &geom) 
{
  ...
  try {
    geom_construct.Delete(subtract_result);
  }
  catch (TSTTB::Error err) {
    std::cerr << "Problems deleting for booleans subtract test." << std::endl;
    return false;
  }
  ...
  try {
    geom_booleans.Section(brick, 1.0, 0.0, 0.0, 0.25, true, section_result);
  }
  catch (TSTTB::Error err) {
  ...
  try {
    geom_construct.Delete(section_result);
  }
  catch (TSTTB::Error err) {
    std::cerr << "Problems deleting for booleans section test." << std::endl;
    return false;
  }
  
    // unite the brick and cylinder
  GentityHandle unite_results;
  sidl::array<GentityHandle> unite_input = sidl::array<GentityHandle>::create1d(2);
  unite_input.set(0, brick);
  ...
  try {
    ...
    std::cerr << "Problems deleting brick after booleans check." << std::endl;
    return false;
  }
