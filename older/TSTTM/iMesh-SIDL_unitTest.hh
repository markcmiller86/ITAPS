#include "iMesh-SIDL_unitTest-config.h"
...
#define CATCH_BAD_ARRAY_SIZE catch(iBase::Error TErr) {\
  TEST(TErr.getErrorType() == iBase::ErrorType_BAD_ARRAY_SIZE);\
}
#define CATCH_NIL_ARRAY catch(iBase::Error TErr) {\
  TEST(TErr.getErrorType() == iBase::ErrorType_NIL_ARRAY);\
}

#define MUST_THROW(ErrType) catch ( iBase::Error& e ) {\
...
#define SHOULD_NOT_THROW catch(iBase::Error TErr) {\
  TErr.echo("Should not have thrown an error!");\
  TEST ( error_throwing );\
}

#define SHOULD_NOT_THROW_CRITICAL catch(iBase::Error TErr) {\
  TErr.echo("Should not have thrown an error!");\
  TEST ( error_throwing );\
...
int iNPassed = 0, iNFailed = 0,
  iNSkipped = 0, iNIntPassed = 0, iNIntSkipped = 0, iNIntFailed = 0;
...
iBase::TagValueType aTagTypes[] = {iBase::TagValueType_INTEGER,
				   iBase::TagValueType_DOUBLE,
				   iBase::TagValueType_ENTITY_HANDLE,
				   iBase::TagValueType_BYTES};
...
#else
extern int iNPassed, iNFailed, iNSkipped, iNIntPassed, iNIntSkipped, iNIntFailed;
...
extern iBase::TagValueType aTagTypes[];
