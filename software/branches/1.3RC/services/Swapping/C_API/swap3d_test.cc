...
int main(int argc, char *argv[] )
{
  ...
  if (argc == 3) {
    ...
    if (strTests.find("t") != std::string::npos) {
      ...
      int qMoreData;
      iMesh_initEntIter(Mesh, root_set, iBase_FACE, iMesh_ALL_TOPOLOGIES,
			&iter, &err);
      CHECK_ERR(err, Mesh);
	...
	std::cout << "Old face that wasn't found: " << VT.vert0 << " "
      ...
      iBase_EntityHandle entity_handle;
      int qMoreData;
      iMesh_initEntIter(Mesh, root_set, iBase_REGION, iMesh_ALL_TOPOLOGIES,
			&iter, &err);
      CHECK_ERR(err, Mesh);
