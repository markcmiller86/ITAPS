...
export ITAPS_FMDB_VERSION="1.3.2"
...
export FORTRAN_TESTS_PATH="https://redmine.scorec.rpi.edu/anonsvn/itaps/software/${ITAPS_BRANCH}/interfaces/iMesh/examples"

export FORTRAN_IMESH_HDR="https://redmine.scorec.rpi.edu/anonsvn/itaps/software/${ITAPS_BRANCH}/interfaces/iMesh/spec/iMesh_f.h"

export FORTRAN_IBASE_HDR="https://redmine.scorec.rpi.edu/anonsvn/itaps/software/${ITAPS_BRANCH}/interfaces/iBase/spec/iBase_f.h"
...
#     before building FMDB - set ENABLE_IMESH to 1
...
# *****************************************************************************
#     Section 12 - Fortran Test Routines
# -----------------------------------------------------------------------------
# *****************************************************************************
# Function: get_source_fort
#
# Purpose: extract sources into fortran_test_dir
#
# Programmer: Ellen Hill
# Date: Tue Apr 5 2011
#
...
function get_source_fort
{
    echo "Getting Fortran Test sources from ${FORTRAN_TESTS_PATH}. . . "
    
    svn co ${FORTRAN_TESTS_PATH} fortran_test_dir \
           1> FORT.checkout 2> FORT.checkout.err
    retval=$?

#     remove checkout file(s) if empty
    find . -name 'FORT.checkout' -size 0 -exec rm -f {} \;
    find . -name 'FORT.checkout.err' -size 0 -exec rm -f {} \;

#
#     make tar file of fortran source code 
    tar -cf fortTest.tar fortran_test_dir/

    export FORT_BUILD_DIR="$(pwd)/fortran_test_dir"
#
#     get list of directories within the fortran test directory
    export FORTRAN_TEST_DIRS=`ls ${FORT_BUILD_DIR}`

#
#     get fortran header files and copy into fortran test direcories
   wget ${FORTRAN_IMESH_HDR} 1> iMesh_f.checkout 2> iMesh_f.checkout.log
   wget ${FORTRAN_IBASE_HDR} 1> iBase_f.checkout 2> iBase_f.checkout.log
...
#     copy *_f.h files to fortran test directories
    for fdir in ${FORTRAN_TEST_DIRS}
    do
       cp -f iMesh_f.h ${FORT_BUILD_DIR}/${fdir}
       cp -f iBase_f.h ${FORT_BUILD_DIR}/${fdir}
    done
...
# *************************************************************************** #
# Function: build_check_fort
#
# Purpose: Builds and runs Fortran Tests
#
# Programmer: Ellen Hill
# Date: Fri Apr 1 2011
# *************************************************************************** #
function build_check_fort
{
    cd $FORT_BUILD_DIR || echo "Cannot cd to $FORT_BUILD_DIR build dir."
    ...
    echo ""
    echo "Building fortran tests for ${IMPL_NAME} . . ."
   
    for tdir in ${FORTRAN_TEST_DIRS}
    do
       if [ -d ${tdir} ]
       then
          if [ "${tdir}" = "MeshFiles" ]
          ...
          cd ${tdir}
          rm -rf *.o
          make -f Makefile.noautoconf   \
               IMESH_DIR=${IMPL_DEFS_DIR}/.. \
               FCFLAGS="-fcray-pointer"  FFLAGS="-fcray-pointer" \
               FC=f95 F77=f77 \
          ...
          if [ "${tdir}" = "perftest" ]
          then
             ./perfF77 1> ${tdir}F77_${IMPL_NAME}.out 2> ${tdir}F77_${IMPL_NAME}.msg 
             ./perfF90 1> ${tdir}F90_${IMPL_NAME}.out 2> ${tdir}F90_${IMPL_NAME}.msg 
          else
             ./${tdir}F77 1> ${tdir}F77_${IMPL_NAME}.out 2> ${tdir}F77_${IMPL_NAME}.msg 
             ./${tdir}F90 1> ${tdir}F90_${IMPL_NAME}.out 2> ${tdir}F90_${IMPL_NAME}.msg 
          fi
       fi
       cd ${FORT_BUILD_DIR}
    done

    echo "Done running Fortran tests for ${IMPL_NAME}"
    echo ""
...
function create_exec_summary
{
   ...
   printf "<caption>iMesh ${ITAPS_BRANCH} Test Results Summary</caption>\n" >> $EXEC_SUMM
   printf "\n" >> $EXEC_SUMM
...
#     print table column headings and test results
   ...
   do
      ...
      case $indx in
        ...
        19) optnam="run"
            ;;
        20) optnam="make"  #start of fortran Tests - FindAdjaceny
            ;;
        21) optnam="F77"
            ;;
        22) optnam="F90"
            ;;
        23) optnam="make"  #FindConnect
            ;;
        24) optnam="F77" 
            ;;
        25) optnam="F90"
            ;;
        26) optnam="make"  #HelloMesh
            ;;
        27) optnam="F77" 
            ;;
        28) optnam="F90"
            ;;
        29) optnam="make"  #ListSetsNTags
            ;;
        30) optnam="F77"
            ;;
        31) optnam="F90"
...
#     print header for each section of tests
      ...
      then
         echo "<th colspan=\"5\">- - - - - Fortran Tests - - - - -<br /> - - - - - FindAdjacency Tests - - - - - </th>" >> $EXEC_SUMM
      elif [[ $indx -eq 23 ]]
      then
...
function create_summary
{
   ...
   printf "<PRE>\n" >> $SUMFILE
   printf "<body>\n" >> $SUMFILE
   printf "ITAPS ${ITAPS_BRANCH} Error Summary for `date`<br>" >> $SUMFILE
   printf "%s<br>" "-----------------------------------------------" >> $SUMFILE
...
#
####     Fortran Test Results
   printf "Fortran Tests Compile Summary:<br>" >> $SUMFILE
   printf "                                 Status: Warnings: Errors<br>" >> $SUMFILE

#     get compile related error files
   errfils=`ls ${FORT_BUILD_DIR}/*/*.err`
   ...
   printf "%s<br>" "-----------------------------------------------" >> $SUMFILE
   printf "Fortran Tests Execution Summary: <br>" >> $SUMFILE

#     get compile related error files
   msgfils=`ls ${FORT_BUILD_DIR}/*/*.msg`
   ...
   do
      ...
      printf "   %-29s" ${fnam} >> $SUMFILE
      if [[ "$errflg" = "999" ]] ; then
         printf "Case did NOT run<br>" >> $SUMFILE
      elif [[ "$errflg" = "1" ]] ; then
         printf " For error messages see: ${finfo}<br>" >> $SUMFILE
      elif [ "$errflg" = "0" ] ; then
         printf " For detailed information see: ${finfo}<br>" >> $SUMFILE
      fi
   done   #  end of errfils loop
...
### - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
###     Building Fortran tests with each implementation
...
then
   get_source_fort
   err_get_fort="$?"
   checkHandleError "$err_get_fort" "Fortran test codes"
   ...
   then
      ...
      do
         ...
         build_check_fort ${impl_name} ${impl_dir}
         retval=$?
...
#     if msg fiel contains "No such"; code did not run
               ck_dnr=`grep -i -e "No such" ${mnam77}|wc -l`
               if [ ${ck_dnr} > 0 ]
               then
                  filarr[$indfr]="DNR"
                  indfr=`expr $indfr + 1`
                  ...
                  filarr[$indfr]=""
                  if [[ ! -z "$(cat ${mnam77})" ]]    #  F77 msg is not empty
                  then
                     numwarn=`grep -i -e "warn" ${mnam77}|wc -l`  # check for warnings
                     numerr=`grep -i -e "error" ${mnam77}|wc -l`  # check for errors
                     if [[ "$numwarn" != "0" ]] && [[ "$numerr" = "0" ]]
                     then
...
#     if msg fiel contains "No such"; code did not run
               ck_dnr=`grep -i -e "No such" ${mnam90}|wc -l`
               if [ ${ck_dnr} > 0 ]
               then
                  filarr[$indfr]="DNR"
                  indfr=`expr $indfr + 1`
                  ...
                  filarr[$indfr]=""
                  if [[ ! -z "$(cat ${mnam90})" ]]    #  F90 msg is not empty
                  then
                     numwarn=`grep -i -e "warn" ${mnam90}|wc -l`  #check for warnings
                     numerr=`grep -i -e "error" ${mnam90}|wc -l`  #check for errors
                     if [[ "$numwarn" != "0" ]] && [[ "$numerr" = "0" ]]
                     then
...
# - - - -    reset implementation indicies
         ...
         case $retval in
            35) checkHandleError $retval "Fortran/${impl_name} make" \
                                "${FORT_BUILD_DIR}/fort_${impl_name}.make.err"
                ;;
            55) checkHandleError $retval \
                           "Fortran/${impl_name}; problem with build of ${impl_name}"
                ;;
             *) checkHandleError $retval "Return value from Fortran/${impl_name}"
                ;;
         esac
...
if [[ "${GET_FORT}" = "yes" ]]
then
   mv -f ${FORT_BUILD_DIR}/*/*.err ${save_dir}
   mv -f ${FORT_BUILD_DIR}/*/*.log ${save_dir}
   mv -f ${FORT_BUILD_DIR}/*/*.msg ${save_dir}
   mv -f ${FORT_BUILD_DIR}/*/*.out ${save_dir}
fi

mv -f ${CurrentDirectory}/buildF* ${save_dir}
mv -f ${CurrentDirectory}/build_S* ${save_dir}
mv -f ${CurrentDirectory}/*_f.h ${save_dir}
