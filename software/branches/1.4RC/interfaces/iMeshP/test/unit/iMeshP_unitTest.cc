...
#endif
/*
    iCurNFailed=iNFailed; //  -- turned off temporarily due to memory corruption
...
*/
  ...
  {
    fprintf(fp, "\n*********************************************");
    fprintf(fp, "\n 5- Testing IMESHP MIGRATION FUNCTIONALITY");
    fprintf(fp, "\n*********************************************\n");
#ifdef DEBUG
    if (!myrank)
    {      
...
*/
  ...
  {
    fprintf(fp, "\n*********************************************");
    fprintf(fp, "\n 7- Testing IMESHP MODIFICATION FUNCTIONALITY");
    fprintf(fp, "\n*******************************************");
#ifdef DEBUG
    if (!myrank)
    {
...
static int vSetupImplStatus()
{
  ...
  test_map["iMeshP_createPart"]=false;impl_map["iMeshP_createPart"]=false;
  test_map["iMeshP_destroyPart"]=false;impl_map["iMeshP_destroyPart"]=false;
  ...
  test_map["iMeshP_waitForRequestEnt"]=false;impl_map["iMeshP_waitForRequestEnt"]=false;
  ...
  test_map["iMeshP_exchEntArrToPartsAll"]=false;impl_map["iMeshP_exchEntArrToPartsAll"]=false;
  test_map["iMeshP_migrateEntity"]=false;impl_map["iMeshP_migrateEntity"]=false;
      ...
      for (eEType = iBase_VERTEX; eEType <= iBase_ALL_TYPES; ++eEType) 
      {
 	  ...
 	  iMeshP_getPartBdryEnts (instance, pPtnHandles[p], pLocalParts[pr],
          ...
          iMeshP_getNumPartBdryEnts (instance, pPtnHandles[p], pLocalParts[pr],
                  static_cast<iBase_EntityType>(eEType), eETopo, iMeshP_ALL_PARTS,
                  &iNumPartBdryEnt, &ierr); 
          TEST(!ierr, _FUNC_, "iMeshP_getNumPartBdryEnts");
          TEST(pPartBdryEntArrSize==iNumPartBdryEnt, _VALID_, "pPartBdryEntArrSize==iNumPartBdryEnt");
          UT_FREE(pPartBdryEntArr);
        } // for eTopo
      } // for eType
    } // for each part
    ...
    for (int ph = 0; ph < pLocalPartSize; ++ph) 
    {
      for (eEType = iBase_VERTEX; eEType <= iBase_ALL_TYPES; ++eEType) 
      {
        ...
        for (int nbor = 0; nbor < pPartNborIdArrSize; ++nbor)
        {
          ...
          TEST(!ierr, _FUNC_, msg);
          TEST(iNumPartBdryEnt>0, _VALID_, "iNumPartBdryEnt>0");
        } // for nbor
