...
#define CAST_ITAPS_INTERFACE(var_in, var_out, iface) \
          iBase::iface var_out; \
          try {var_out = sidl::babel_cast<iBase::iface>(var_in);}\
          catch(iBase::Error) {\
...
#define CAST_IMESH_INTERFACE(var_in, var_out, iface) \
          iMesh::iface var_out; \
          try {var_out = sidl::babel_cast<iMesh::iface>(var_in);}\
          catch(iBase::Error) {\
            cerr << "Error: current interface doesn't support iface." << endl; \
            return false;}
/*!
...
@li Load a mesh file
*/
bool load_mesh_test(const std::string filename, iMesh::Mesh &mesh, void* root_set)
{
  // load a mesh
  std::string options;
  try {
    mesh.load(root_set, filename, options);
  }
  catch (iBase::Error) {
...
*/
bool topology_dimension_test(iMesh::Mesh &mesh, void* root_set)
{
  // first get 2D entities
  sidl::array<Entity_Handle> faces;
  int faces_size;
  try {
    mesh.getEntities(root_set, iBase::EntityType_FACE, 
...
bool topology_adjacency_test(iMesh::Mesh &mesh, void* root_set)
{
  CAST_IMESH_INTERFACE(mesh, mesh_arr, Arr);
  ...
  for (; top < num_test_top; top++) {
    ...
    int entities_size;
    try {
      mesh.getEntities(root_set, iBase::EntityType_ALL_TYPES,
  ...
  for (int top_j = 0; top_j < num_test_top; top_j++) {
    int num_tops = 0;
    try {
      mesh.getNumOfTopo(root_set, static_cast<iMesh::EntityTopology>(top_j), 
                        num_tops);
    } catch (iBase::Error err) {
...
bool entity_connectivity_test(iMesh::Mesh &mesh, void* root_set)
{
  CAST_IMESH_INTERFACE(mesh, mesh_arr, Arr);
  ...
  for (; type < iBase::EntityType_ALL_TYPES; type++) {
    ...
    int indices_size;
    sidl::array<void*> entities1, adj_entities;
    int entities1_size, adj_entities_size;
    
    try {
      mesh.getAdjEntIndices(root_set, static_cast<iBase::EntityType>(type), 
                            iMesh::EntityTopology_ALL_TOPOLOGIES,
                            iBase::EntityType_VERTEX,
                            entities1, entities1_size, 
                            adj_entities, adj_entities_size,
    ...
    if (ARRAY_SIZE(entities1) != entities1_size) {
      cerr << "Number of entities didn't agree with array size in connectivity_test."
           << endl;
      return false;
    }

    if (ARRAY_SIZE(adj_entities) != adj_entities_size) {
      cerr << "Number of adj_entities didn't agree with array size in connectivity_test."
           << endl;
      return false;
    }
    ...
    try {
      mesh.getAdjEntities(root_set, static_cast<iBase::EntityType>(type), 
...
bool check_esets(iBase::EntSet &mesh_eset, const int num_sets, void* root_set);

bool entity_sets_subtest(iMesh::Mesh &mesh, bool is_list,
                         int num_iter, void* root_set)
{
  ...
  int n_whole_mesh = 0;
  try {
    mesh_eset.getNumEntSets(root_set, 1, n_whole_mesh);
  } catch (iBase::Error) {
  ...
  for (; ent_type < num_type; ent_type++) {
    ...
    int entities_size = 0;
    try {
      mesh.getEntities(root_set, static_cast<iBase::EntityType>(ent_type),
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type, root_set)) return false;
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type + 1, root_set)) return false;
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type + 2, root_set)) return false;
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type + 4, root_set)) return false;
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type + 5, root_set)) return false;
  ...
  int adj_faces_size;
  sidl::array<int> face_offsets;
  int face_offsets_size;

  try {
    mesh.getAdjEntities(root_set, iBase::EntityType_ALL_TYPES,
  ...
  if (!check_esets(mesh_eset, n_whole_mesh + num_type + 6, root_set)) return false;
...
bool check_esets(iBase::EntSet &mesh_eset, const int num_sets, void* root_set) 
{
  ...
  try {
    mesh_eset.getNumEntSets(root_set, 1, entity_sets_size);
  } catch (iBase::Error) {
...
@li Check entity sets
*/
bool entity_sets_test(iMesh::Mesh &mesh, void* root_set)
{
  int iter_num = 0;
  // check 
  for (int i = 0; i < 2; i++) {
      ...
      bool result = entity_sets_subtest(mesh, i, iter_num, root_set);
      if (!result)
	return result;
  }
...
*/
bool vertex_coordinates_test(iMesh::Mesh &mesh, void* root_set)
{
...
//
//  try {
//    mesh.getAllVtxCoords(root_set, all_coords, all_coords_size,
...
//  }
  ...
  int verts_size;
  try {
    mesh.getEntities(root_set, iBase::EntityType_VERTEX, 
...
*/
bool tag_info_test(iMesh::Mesh &mesh, void* root_set)
{
...
bool vertex_tag_test(iMesh::Mesh &mesh, void* root_set)
{
  bool info_err = tag_info_test(mesh, root_set);
  
  // get all the vertices
  sidl::array<void*> verts;
  int verts_size;
  try {mesh.getEntities(root_set, iBase::EntityType_ALL_TYPES, 
                        iMesh::EntityTopology_POINT, verts, verts_size);}
  catch(iBase::Error) {
...
*/
bool entityset_tag_test(iMesh::Mesh &mesh, void* root_set)
{
  ...
  int esets_size;
  try {
    mesh_eset.getEntSets (root_set, 1, esets, esets_size);
  }
  catch (iBase::Error) {
...
bool mesh_int_tag_test(iMesh::Mesh &mesh, void *&int_tag, void* root_set) 
{
  CAST_ITAPS_INTERFACE(mesh, mesh_stag, SetTag);
  ...
  try {
    mesh_stag.setEntSetData(root_set, int_tag, dum_val, dum_val_size);
  }
  catch (iBase::Error) {
  ...
  try {
    mesh_stag.getEntSetData(root_set, int_tag,
                            dum_val, dum_val_size);
  }
  catch (iBase::Error) {
...
bool mesh_double_tag_test(iMesh::Mesh &mesh, void *&double_tag, void* root_set) 
{
  CAST_ITAPS_INTERFACE(mesh, mesh_stag, SetTag);
  ...
  try {
    mesh_stag.setEntSetData(root_set, double_tag, dum_val, dum_val_size);
  }
  catch (iBase::Error) {
  ...
  try {
    mesh_stag.getEntSetData(root_set, double_tag,
                            dum_val, dum_val_size);
  }
  catch (iBase::Error) {
...
bool mesh_struct_tag_test(iMesh::Mesh &mesh, void *&struct_tag, void* root_set) 
{
  CAST_ITAPS_INTERFACE(mesh, mesh_stag, SetTag);
  ...
  try {
    mesh_stag.setEntSetData(root_set, struct_tag, dum_val, dum_val_size);
  }
  catch (iBase::Error) {
  ...
  try {
    mesh_stag.getEntSetData(root_set, struct_tag,
                            dum_val, dum_val_size);
  }
  catch (iBase::Error) {
...
bool mesh_tag_delete_test(iMesh::Mesh &mesh, void* root_set) 
{
  ...
  int all_tags_size;
  try {
    mesh_stag.getAllEntSetTags(root_set, all_tags, all_tags_size);
  }
  catch (iBase::Error) {
  ...
  if (all_tags_size > 0) {
    try {
      mesh_stag.rmvEntSetTag(root_set, all_tags.get(0));
    }
    catch (iBase::Error) {
    ...
    if (all_tags_size > 1) {
      ...
      try {
        ...
        mesh.getEntities( root_set, iBase::EntityType_ALL_TYPES, iMesh::EntityTopology_ALL_TOPOLOGIES, all_ents, num_ent );
        if (num_ent > 0) {
...
@li Check mesh tags
*/
bool mesh_tag_test( iMesh::Mesh &mesh, void* root_set )
{
  void *int_tag, *double_tag, *struct_tag;
  
  // mesh int tag
  bool int_success = mesh_int_tag_test(mesh, int_tag, root_set);
    
  // mesh double tag
  bool double_success = mesh_double_tag_test(mesh, double_tag, root_set);
    
  // mesh struct tag
  bool struct_success = mesh_struct_tag_test(mesh, struct_tag, root_set);
  
  bool tag_delete_success = mesh_tag_delete_test(mesh, root_set);
...
int main( int argc, char *argv[] )
{
  ...
  std::string options;
  try {
//    iMeshSIDL::MeshSidl::newMesh(options, mesh);
    mesh = iMeshSIDL::MeshSidl::_create();
  }
  catch (iBase::Error err) {
  ...
  void* root_set = 0;
  try {
    mesh.getRootSet( root_set );
  }
  catch (iBase::Error err) {
    cout << "Trouble getting root set." << endl;
    return 1;
  }
  ...
  // load_mesh test
  cout << "   load_mesh: ";
  result = load_mesh_test(filename, mesh, root_set);
  ...
  result = topology_adjacency_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = entity_connectivity_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = vertex_coordinates_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = topology_dimension_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = entity_sets_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = vertex_tag_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  result = entityset_tag_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
  ...
  // mesh tag test
  cout << "   mesh_tag_test: ";
  result = mesh_tag_test( mesh, root_set );
  handle_error_code(result, number_tests_failed,
                    number_tests_not_implemented,
                    number_tests_successful);
  number_tests++;
