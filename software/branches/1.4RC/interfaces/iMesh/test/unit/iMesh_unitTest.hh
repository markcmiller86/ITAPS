...
#define TYPE_USAGE do {\
    ...
    cerr << " --no-error                   Don't test all error throwing." << endl; \
...
// can do that!
inline void writeErrorDescr(int err) {
...
bool qAllPassed = true, qErrTests = true, qSilent = false, qOutFile = false;
bool qSaveLoad = true;
...
void vCheckBoolOpResult(iMesh_Instance instance,
         ...
         int iNumEntsExpected);
void CleanSaveLoad();
void CleanSaveLoadOnSignal(int);
