...
[compile for profiling using prof or gprof]))

if (test "X$enable_profiling" = "Xyes") ; then
  dnl Figure out whether gprof or prof will be used
  AC_CHECK_PROG(PROF_FLAGS, gprof, [pg], [])
  if (test "X$PROF_FLAGS" != "Xpg") ; then
    AC_CHECK_PROG(PROF_FLAGS, prof, [g], [])
  fi
  CFLAGS="$CFLAGS -$PROF_FLAGS"
  CXXFLAGS="$CXXFLAGS -$PROF_FLAGS"
  FFLAGS="$FFLAGS -$PROF_FLAGS"
  FCFLAGS="$FCFLAGS -$PROF_FLAGS"
  LDFLAGS="$LDFLAGS -$PROF_FLAGS"
fi

AC_ARG_ENABLE([optimization],
AS_HELP_STRING([--enable-optimization],
[set an optimization level (default = 2)]))

case "$enable_optimization" in
  no|0) CFLAGS="$CFLAGS -O0"
  ...
  yes) CFLAGS="$CFLAGS -O2"
	...
	FFLAGS="$FFLAGS -O2"
	FCFLAGS="$FCFLAGS -O2";;
  1|2|3|*) CFLAGS="$CFLAGS -O$enable_optimization"
	CXXFLAGS="$CXXFLAGS -O$enable_optimization"
	FFLAGS="$FFLAGS -O$enable_optimization"
	FCFLAGS="$FCFLAGS -O$enable_optimization";;
esac
...
AC_CONFIG_SRCDIR(iBase-Defs.inc.in)
AC_CANONICAL_HOST
