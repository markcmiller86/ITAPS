...
int main( int argc, char* argv[] )
{
  ...
  strcpy(FILENAME, "iMeshP_test_file");
  ...
  int num_errors = 0;
  num_errors += RUN_TEST( test_get_partitions, skipCreate );
  num_errors += RUN_TEST( test_get_parts, skipCreate );
  num_errors += RUN_TEST( test_get_by_type, skipCreate );
  num_errors += RUN_TEST( test_get_by_topo, skipCreate );
  num_errors += RUN_TEST( test_part_id_handle, skipCreate );
  num_errors += RUN_TEST( test_part_rank, skipCreate );
  num_errors += RUN_TEST( test_get_neighbors, skipCreate );
  num_errors += RUN_TEST( test_get_part_boundary, skipCreate );
  num_errors += RUN_TEST( test_part_boundary_iter, skipCreate );
  num_errors += RUN_TEST( test_get_adjacencies, skipCreate );
  num_errors += RUN_TEST( test_entity_iterator, skipCreate );
  num_errors += RUN_TEST( test_entity_owner, skipCreate );
  num_errors += RUN_TEST( test_entity_status, skipCreate );
  num_errors += RUN_TEST( test_entity_copy_parts, skipCreate );
  num_errors += RUN_TEST( test_entity_copies, skipCreate );
  num_errors += RUN_TEST( test_push_tag_data_iface, skipCreate );
  num_errors += RUN_TEST( test_push_tag_data_ghost, skipCreate );
  num_errors += RUN_TEST( test_create_ghost_ents, skipCreate );
...
int create_mesh( const char* filename, int num_parts )
{
  ...
  static int q = 0;
  char cmd[256];
  snprintf(cmd, sizeof(cmd), "cp %s %s.%d", filename, filename, q++);
  system(cmd);
#endif
  ...
  for (size_t i = 0; i < local_handles.size(); ++i) {
    ...
    for (std::vector<iMeshP_Part>::iterator j = all_parts.begin(); j != all_parts.end(); ++j) {
      ...
      iMeshP_initPartBdryEntArrIter( imesh, prtn, local_handle, iBase_VERTEX, iMesh_POINT,
                                     num_shared_verts, other_id, resilient, &aiter, &ierr );
      if (ierr != iBase_SUCCESS) {
