...
#define CHECK(a) if (iBase_SUCCESS != ierr) std::cout << a << std::endl, exit(ierr)

int main( int argc, char *argv[] )
{
  int i, ierr;
  iMesh_Instance mesh;
  iBase_EntityHandle *ents = NULL, *verts = NULL, *allverts = NULL;
  int ents_alloc = 0, ents_size;
  int verts_alloc = 0, verts_size;
  int allverts_alloc = 0, allverts_size;
  int *offsets = NULL, offsets_alloc = 0, offsets_size;
  int vert_uses = 0;
  ...
  CHECK("Problems getting root set");
    ...
    // load the mesh
  iMesh_load(mesh, root_set, "../MeshFiles/125hex.vtk", "", &ierr, 23, 0);
  CHECK("Load failed");
