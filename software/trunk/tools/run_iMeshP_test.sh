...
#
# Purpose:
#   The run_iMeshP_test.sh script is used to run the parallel iMesh_unitTest for
#   two ITAPS implementations, MOAB and FMDB, on itaps.mcs.anl.gov machine at 
#   Argonne.  In addition, several services are tested against the two ITAPS 
#   implementations.  The script is divided into several sections each of which
#   is documented below.  Any assumptions made are included in the respective 
#   sections.
#
...
#   filarr, is initialized with the number of implementations (2) and the total 
#   number of options being run; i.e. configure, make, run, etc.  As the script 
#   is currently set up there are 18 options. The array is initialized to ‘ZZZ’,
#   which indicates that an option does not exist if it is the final element 
#   value.  
#
#   There are two functions in this section, usage and find_MPI.  The usage 
#   routine is printed whenever the ‘help’ argument is given on the script command
#   line.  find_MPI determines the name and location of the MPI wrapper codes 
#   and libraries used for C++.
#
#
# Section 2:  File & Error Handling Routines
#   uncompress_untar
#   Uncompress and un-tar source code tar-balls based on the suffix of the file 
#   name.  If the input file name is NULL or ends in “.sh” the input file is not
#   compressed or tarred so this routine exits without an error.
#
#   checkHandleError
...
#   get_source_MOAB
#   The ‘wget’ command is used to retrieve MOAB source code from the specified 
#   location.  The output ‘checkout’ files are monitored to determine when the 
#   download has completed.  When the last line containing the word ‘saved’ is 
#   detected, the download is complete.  If it is not detected after 2 minutes,
#   a download error is reported. 
#
#   If a download error occurred, all further use of MOAB is omitted and the 
...
#
#   build_check_MOAB
#   Runs configure to enable MOAB to run with its parallel iMesh option.  
#   NOTE - MOAB requires the location of the HDF5 library to be included as an 
#   input to configure.  This is hard-wired in the configure step in this routine.
#   If building on a platform other than itaps.mcs.anl.gov this location needs 
#   to be updated.
#
#   MOAB is then built, installed and checked to ensure proper building.  After
#   each step, if there was a failure, no further processing is done and the error
#   array elements are set to ‘DNR’.  If the step was successful, the output log
#   files are checked for warnings which are noted in the relevant error array 
#   element.  The test results are either pass or fail which are also noted in 
#   the relevant error array element.
#
#
# Section 4:  FMDB Routines
#   get_source_FMDB:   
#   The ‘wget’ command is used to retrieve FMDB source code from the specified 
#   location.  The output ‘checkout’ files are monitored to determine when the 
#   download has completed.  When the last line containing the word ‘saved’ is 
#   detected, the download is complete.  If it is not detected after 2 minutes,
#   a download error is reported. 
...
#   source code directory to be in the SCOREC_Software directory and the installed
#   FMDB data to be in a directory named fmdbParallel.  The test script uses 
#   this information to determine the location of the iMesh-Defs.inc and 
#   iMeshP-Defs.inc files.
#
#
# Section 5:  IPComman Routines
#   get_source_IPCOMM
#   Based on specified location of the IPComman source code either ‘svn checkout’
#   or ‘wget’ is used to retrieve the code.  The output ‘checkout’ files are 
#   monitored to determine when the download has completed.  When ‘svn’ is used 
#   the checkout is complete when the last line of the file contains ‘revision’.
#   For ‘wget’ the last line contains ‘saved’ when the download is complete.  
#   In both cases, a 2 minute time limit is set, after which a download error 
#   is reported. 
#
#   If a download error occurred, all further use of IPComman is omitted and the
#   error array elements are set to ‘DNR’.  If the download was successful the 
#   output files are scanned for possible warning messages and the error array 
#   elements are appropriately updated.
#
#   get_builddir_IPCOMM
#   Finds the IPComman source code directory by looking for a directory beginning
#   with ‘IPC’ or ‘ipc’
#
#   build_check_IPCOMM
#   Runs configure with the MPI option.  IPComman is then built and installed. 
#   After each step, if there was a failure, no further processing is done and 
#   the error array elements are set to ‘DNR’.  If the step was successful, the 
...
#
#   build_check_ZOLTAN
#   Runs configure with parallel options.  Zoltan is then built and installed.  
#   After each step, if there was a failure, no further processing is done and 
#   the error array elements are set to ‘DNR’.  If the step was successful, the 
#   output log files are checked for warnings which are noted in the relevant 
#   error array element.  
...
#   Finds the source code directory for ParMETIS, which is distributed with 
#   Zoltan, by searching for ‘Par’ or ‘par’
#
#   build_ ZOLT_PARM
#   Build the ParMETIS library that is needed for Zoltan.  The make command for 
#   ParMETIS does a check that will fail even though the libraries were properly
#   built.  This is overridden by checking for the libraries within the test 
#   script and setting the return value to ‘pass’. 
#
#
# Section 7:  UnitTestP Routines
#   get_source_UnitTest P
#   The iMeshP_unitTest code is checked out of the code repository using 
#   ‘svn checkout’ and stored it in the local imeshp_unitTest_dir directory.  
#   The iMesh, iMeshP and iBase header files are retrieved and moved into the 
#   source code directory.
#
#   get_builddir_UnitTestP
#   The source code directory for the iMeshP_unitTest codes is defined to be 
#   imeshp_unitTest_dir.  This name is set in the get_source_UnitTestP routine 
#   when the code is checked out.
#
#   create_iMeshP_Make
#   There are two input arguments, the implementation name and directory for 
#   the iMeshP-Defs.inc file.  The makefile is created for the specified 
#   implementation using the template provided with the iMeshP_unitTest source 
#   code.  
#
#   build_run_UnitTestP
...
#   Makefile is created for the specified implementation then the iMeshP_unitTest
#   code is configured, built and run separately for each implementation, 
#   i.e. MOAB and FMDB.  By default the make command does some comparisons with 
#   the iMeshP specification.  If this fails, the option to make without checking
#   the specification is automatically run.  If either make command is 
#   successful, the test cases are run with and without valgrind.  Each step 
#   (configure, make, test and test-valgrind) has an error flag associated with 
#   it.  If the step failed the flag is set to 1 otherwise it is set to 0.  These
#   values are used upon exiting this routine to properly set the respective 
#   error array elements.
# 
#   run_MOAB_unitTest
#   The MOAB_iMeshP_unitTest code is provided by the MOAB team in order to check
#   their iMeshP implementation.  It can be run with either MOAB or FMDB.  This 
#   routine has two input arguments, the implementation name and its build 
#   directory.  TheMakefile is created for the specified implementation then the
#   MOAB_iMeshP_unitTest code is configured, built and run separately for each 
#   implementation, i.e. MOAB and FMDB.  If the make command is successful, the 
#   test cases are run with and without valgrind.  Each step (configure, make, 
#   test and test-valgrind) has an error flag associated with it.  If the step 
#   failed the flag is set to 1 otherwise it is set to 0.  These values are used
#   upon exiting this routine to properly set the respective error array elements.
#
#
...
#   Finds the iZoltan-parallel source code directory by looking for a directory 
#   beginning with ‘IZo’ or ‘izo’
#
#   build_check_izoltanp 
#   This routine has three input arguments, the implementation name, its build 
#   directory and the name of its iMeshP-Defs.inc file.  The izoltanp code is 
#   configured, built and run separately for each implementation, i.e. MOAB and 
#   FMDB.  After a successful configure and make the self-contained tests are 
#   run.  If the return value from the tests is non-zero, there was at least one
#   failure, otherwise the tests passed.  Each step (configure, make, test) has 
#   an error flag associated with it.  If the step failed the flag is set to 1 
...
#   build_check_MESQ
#   This routine has three input arguments, the implementation name, its build 
#   directory and the name of its iMeshP-Defs.inc file.  The Mesquite code is 
#   configured, built and run separately for each implementation, i.e. MOAB and 
#   FMDB.  After a successful configure and make the self-contained tests are run.
#   If the return value from the tests is non-zero, there was at least one 
#   failure, otherwise the tests passed.  Each step (configure, make, test) has 
#   an error flag associated with it.  If the step failed the flag is set to 1 
#   otherwise it is set to 0.  These values are used upon exiting this routine 
#   to properly set the respective error array elements.
...
#   *.err file is scanned for ‘warning’ and ‘error’ with the total number of each
#   being counted.  The output *.out and *.msg files are scanned to determine 
#   the results of the tests.  Each section that reports test results is somewhat
#   customized to the specific output format of the code itself, therefore there
#   are different searches being done.  For example, Mesquite notes failed tests
#   with ‘FAIL’ while some of the example codes report ‘No such file’.  For each
#   test that failed there is a clickable link to the specific output file to 
#   view more details. 
#
#   create_html_index
...
#         Determine source code directory
#         If successful finding source code directory
#            Build & check IPComman
#
#   Building Zoltan & ParMETIS
#   Get Zoltan source code – which contains ParMETIS source code
#   If successful getting code
#      Untar file
#      If successful untar
#         Determine ParMETIS source code directory
#         If successful finding source code directory
#            Untar file
#            If successful untar
#               Build and check ParMETIS 
#               If successful ParMETIS build
...
#         Determine source code directory
#         If successful finding source code directory
#            Run MOAB_unitTestP for each implementation
#            Determine pass/fail/warn/DNR and set error array elements
#
#   Building iZoltan-parallel
#   Get source code
#   If successful getting code
#      Untar file
#      If successful untar
#         Determine source code directory
#         If successful finding source code directory
#            Run iZoltan-parallel for each implementation
#            Determine pass/fail/warn/DNR and set error array elements
#
#   Building Mesquite
#   Get source code
...
#
#   Ellen M. Hill, Wed Apr 04 15:48:47 PDT 2012
#   Clean-up for consistency with documentation and ITAPS release 1.4
#
#   Ellen M. Hill, Mon Apr 16 10:42:16 PDT 2012
#   Added documentation details above.
# *****************************************************************************
...
#     save IPCOMM make-install error return value & file name 
#     (if non-zero length)
    if [[ "$retval" != "0" ]] 
    then
       echo "IPComman make failed, Giving up"
...
#     if ZOLTAN.config failed, set ZOLTAN.make to N/A and ZOLTAN.make-install
#     to N/A
    if [[ "$retval" != "0" ]] 
    then
...
#
#     create temporary file; add tab character as needed for make; replace 
#     original Makefile.iMeshP with temporary file
    tmpf="tmpfil"
    while read InpL
    do
       case $InpL in
      ...
      then
         ...
         do
            ...
            then
               ...
               all_pass=`grep -i -e "all tests passed" ${IMESHP_BUILD_DIR}/${fil} | wc -l`
               if [ "$all_pass" = "0" ]
               then 
                  echo "     Some/All tests FAILED" >> $SUMFILE
               else
